...
def ItemFill(item, options, feedurl="/", fast=False):
    ...
    if "engine" in options and options.engine == "jina":
        out = jina_reader.get_article(item.link)
        item.content = out
        return True
    ...
    try:
        req = crawler.adv_get(
            url=item.link, policy=policy, force_min=24 * 60 * 60, timeout=TIMEOUT
        )
    ...
    if (
        req["contenttype"] not in crawler.MIMETYPE["html"]
        and req["contenttype"] != "text/plain"
    ):
        log("non-text page")
        return True
...
def FeedFetch(url, options):
    ...
    try:
        req = crawler.adv_get(
            url=url,
            post=options.post,
            follow=("rss" if not options.items else None),
            policy=policy,
            force_min=5 * 60,
            force_max=60 * 60,
            timeout=TIMEOUT,
...
def FeedGather(rss, url, options):
    ...
    if options.order == "last":
        ...
        sorted_items = sorted(
            sorted_items, key=lambda x: x.updated or x.time or now
        )  # oldest to newest
...
def FeedFormat(rss, options, encoding="utf-8"):
    if options.callback:
        ...
        if options.indent:
            return rss.torss(
                xml_declaration=(not encoding == "unicode"),
                encoding=encoding,
                pretty_print=True,
            )
            ...
            return rss.torss(
                xml_declaration=(not encoding == "unicode"), encoding=encoding
            )
