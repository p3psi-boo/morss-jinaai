...
def parse_options(options):
    ...
    for option in options:
        ...
        if len(split) > 1:
            out[split[0]] = unquote(split[1]).replace(
                "|", "/"
            )  # | -> / for backward compatibility (and Apache)
...
def cgi_get(environ, start_response):
    ...
    if options["get"] in ("page", "article"):
        ...
        if req["contenttype"] in crawler.MIMETYPE["html"]:
            if options["get"] == "page":
                if options.get("engine") == "jina":
                    from . import jina_reader

                    output = jina_reader.get_article(
                        url, options.get("jina_format", "markdown")
                    )
                else:
                    ...
                    if html is not None:
                        ...
                        output = lxml.etree.tostring(
                            html, encoding="utf-8", method="html"
                        )
                    else:
                        output = b""
                ...
                if options.get("engine") == "jina":
                    ...
                    output = jina_reader.get_article(url)
                else:
            ...
            req["contenttype"]
            in crawler.MIMETYPE["xml"]
            + crawler.MIMETYPE["rss"]
            + crawler.MIMETYPE["json"]
        ):
            output = req["data"]
